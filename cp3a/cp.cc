...
using f64 = double;
using f64x8 __attribute__ (( vector_size((64)) )) = f64;

static constexpr u32 B1 = 112; // 2048;
...
void correlate(int _ny, int _nx, const float *data, float *result) {
  ...
  if (ny * nx < 100) {
    alignas(64) static f64 _data[100];
    for (u32 y = 0; y < ny; y++) {
      f64 sum = 0;
      for (u32 x = 0; x < nx; x++)
        sum += data[x + y*nx];
  ...
  alignas(64) static f64 __data[5100 * 5100];
  alignas(64) static f64 __result[5100 * 5100];

  f64* _data = __data;
  f64* _result = __result;
  
  if (my * std::max(my, mx) > 5100 * 5100) {
    _data = static_cast<f64*>(std::aligned_alloc(64, my * mx * sizeof(f64)));
    _result = static_cast<f64*>(std::aligned_alloc(64, my * my * sizeof(f64)));
  }
  ...
  for (u32 x = 0; x < mx; x += B1) {
    for (u32 j = 0; j < my; j += B2) {
      ...
      for (u32 i = j; i < my; i += B2) {
        f64x8 result_reg[B2][B2] = {};

        for (u32 k = 0; k < B1; k += 8) {
          f64x8 i_reg[B2], j_reg[B2];

          #pragma GCC unroll 4
          for (u32 l = 0; l < B2; l++) {
            i_reg[l] = *(f64x8*)&_data[x+k + (i+l)*mx];
            j_reg[l] = *(f64x8*)&_data[x+k + (j+l)*mx];
          }
        ...
        for (u32 jj = 0; jj < B2; jj++) {
          ...
          for (u32 ii = 0; ii < B2; ii++) {
            _result[i+ii + (j+jj)*my] +=
                _mm512_reduce_add_pd(result_reg[jj][ii]);
          }
        }
      }
    }
